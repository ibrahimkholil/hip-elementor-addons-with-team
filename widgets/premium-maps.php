...
class Premium_Maps extends Widget_Base
{
	...
	{
		return sprintf('%1$s', __('HIP Google Maps', 'premium-addons-for-elementor'));
	}
	...
	public static function getLocations()
	{
		...
		foreach ($posts as $post) {
			array_push(
				$locations,
				array(
					'map_latitude'  => $post->location_lat,
					'map_longitude' => $post->location_lng,
	...
	{ // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		if (empty($settings['hip-map-api']) || '1' == $settings['hip-map-api']) { // phpcs:ignore WordPress.PHP.StrictComparisons
			$this->add_control(
		...
		$this->add_control(
			...
			array(
				...
				'return_value' => 'true',
				'condition' => array(
					'premium_map_fit_bounds!' => 'yes',
				),
			)
		);

		$this->add_control(
			'premium_map_fit_bounds',
			array(
				'label' => __('Auto Fit Map Bounds', 'premium-addons-for-elementor'),
				'type' => Controls_Manager::SWITCHER,
				'condition' => array(
		...
		$this->add_control(
			'premium_maps_location_map_pins',
			array(
				'label'       => __('Location Map Pins', 'premium-addons-for-elementor'),
				'type'        => Controls_Manager::REPEATER,
				'default'     => \HipAddons\Widgets\Premium_Maps::getLocations(),
				'fields'      => $repeater->get_controls(),
				'title_field' => '{{{ pin_title }}}',
				'prevent_empty' => false,
				'condition' => array(
		...
		$this->add_control(
			'premium_maps_map_skin',
			array(
				'label'   => __('Map Skin', 'premium-addons-for-elementor'),
				'type'    => Controls_Manager::SELECT,
				'options' => array(
					'standard'   => __('Standard', 'premium-addons-for-elementor'),
					'silver'    => __('Silver', 'premium-addons-for-elementor'),
					'retro'     => __('Retro', 'premium-addons-for-elementor'),
					'dark'     => __('Dark', 'premium-addons-for-elementor'),
					'night'     => __('Night', 'premium-addons-for-elementor'),
					'aubergine'     => __('Aubergine', 'premium-addons-for-elementor'),
					'aqua'     => __('Aqua', 'premium-addons-for-elementor'),
					'classic_blue'     => __('Classic Blue', 'premium-addons-for-elementor'),
					'earth'     => __('Earth', 'premium-addons-for-elementor'),
					'magnesium' => __('Magnesium', 'premium-addons-for-elementor'),
					'custom' => __('Custom', 'premium-addons-for-elementor'),
				),
				'default' => 'standard',
				'condition' => array(
					'premium_maps_map_type!' => 'satellite',
				),
			)
		);
	...
	protected function render()
	{
		...
		$location_map_pins = $settings['premium_maps_location_map_pins'];

		if (!empty($settings['premium_maps_location_map_pins'])) {
		...
		$fit_bounds = 'yes' === $settings['premium_map_fit_bounds'] ? 'true' : 'false';
		...
		$map_settings = array(
			...
			'mapskin'           => $settings['premium_maps_map_skin'],
						...
						<div <?php echo wp_kses_post($this->get_render_attribute_string($key)); ?>>
							<?php if (!empty($pin['pin_title']) || !empty($pin['pin_desc'])) : ?>
								<div class='premium-maps-info-container'>
									...
									<div class='premium-maps-info-desc'><?php echo wp_kses_post($pin['pin_desc']); ?></div>
								</div>
							<?php endif; ?>
						</div>
					<?php
